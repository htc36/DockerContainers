...
class Mapping(nn.Module):
    ...
    def __init__(
        ...
        self.mapper = nn.Linear(embed_size, ep_len *
                                embed_size).to(self.device)
...
class Net(nn.Module):
    ...
    def forward(self, img, temperature=1.0):
        ...
        with torch.no_grad():
            ...
            sos_emb = self.td.model.transformer.wte(torch.tensor(
                self.td.tokenizer.bos_token_id).to(self.device))
...
if __name__ == '__main__':
    for clip, text in [['openai/clip-vit-base-patch32', 'gpt2'], ['openai/clip-vit-large-patch14', 'gpt2-medium']]:
        ...
        print(
            f'Total number of parameters: {sum(p.numel() for p in m.parameters())}')
        print(
